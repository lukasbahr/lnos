...
def trainAutonomousAutoencoder(data, observer, options):
    ...
    for trajectorie in range(data.shape[2]):
        # Create trainloader
        trainloader = utils.data.DataLoader(data[:, :, trajectorie].T, batch_size=options['batchSize'],
                                            shuffle=options['shuffle'], num_workers=2, drop_last=True)
        ...
        for epoch in range(options['epochs']):
            ...
            for i, batch in enumerate(trainloader, 0):
                ...
                labels = torch.tensor(batch[:,observer.dim_x+1:]).to(device)
